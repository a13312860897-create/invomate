  ...
  try {
    ...
    if (token === 'dev-mock-token') {
      try {
        ...
        const latestUser = memoryDb.findUserById(1);
        
        if (latestUser) {
          console.log('🔍 从内存数据库获取最新用户信息:', {
            id: latestUser.id,
            subscriptionEndDate: latestUser.subscriptionEndDate,
            subscriptionStatus: latestUser.subscriptionStatus
          });
          req.user = latestUser;
        } else {
          // 如果内存数据库中没有，使用默认值
          console.log('⚠️ 内存数据库中未找到用户，使用默认值');
          req.user = {
            id: 1,
            firstName: '用户',
            lastName: '测试',
    ...
    if (process.env.DB_TYPE === 'memory') {
      try {
        ...
        // JWT验证失败，继续到下面的正常验证流程
        console.log('内存数据库模式下JWT验证失败:', error.message);
      }
    }
...
const checkFeatureAccess = (feature) => {
  const featureMatrix = {
    ...
    'email_sending': ['basic', 'pro', 'enterprise']  // 添加邮件发送功能，所有订阅都可以使用
  };
...
const checkInvoiceOwnership = async (req, res, next) => {
  try {
    ...
    const hasSequelize = !!models.sequelize;

    // 内存模式：优先查内存库，找不到则回退到Sequelize
    if (process.env.DB_TYPE === 'memory') {
      const memoryDb = require('../config/memoryDatabase');
      let invoice = memoryDb.findInvoiceById(invoiceId);

      if (!invoice && hasSequelize) {
        try {
          invoice = await models.Invoice.findOne({ where: { id: invoiceId, userId } });
        } catch (e) {
          // 忽略ORM回退异常，保持后续统一404处理
        }
      }
