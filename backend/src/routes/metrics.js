...
router.get('/summary', async (req, res) => {
  try {
    ...
    if (isMemoryMode) {
      ...
      const totalRevenue = paidInvoices.reduce((sum, inv) => sum + (parseFloat(inv.totalAmount || inv.total) || 0), 0);
      
      const pendingInvoices = allInvoices.filter(inv => inv.status === 'sent');
      const pendingAmount = pendingInvoices.reduce((sum, inv) => sum + (parseFloat(inv.totalAmount || inv.total) || 0), 0);
      
      const now = new Date();
      const overdueInvoices = allInvoices.filter(inv => 
        inv.status !== 'paid' && new Date(inv.dueDate) < now
      );
      const overdueAmount = overdueInvoices.reduce((sum, inv) => sum + (parseFloat(inv.totalAmount || inv.total) || 0), 0);
      ...
      const recentPaymentsAmount = recentPayments.reduce((sum, inv) => sum + (parseFloat(inv.totalAmount || inv.total) || 0), 0);
