...
const pdfService = require('../services/pdfServicePDFKit');
...
router.post('/test-email', async (req, res) => {
  try {
    ...
    // 发送测试邮件（使用 EmailService 实例以兼容当前导出为类）
    const EmailService = require('../services/emailService');
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      memoryDb.invoices.forEach((invoice, index) => {
        console.log(`  ${index + 1}. ${invoice.invoiceNumber} - 状态: ${invoice.status} - 到期: ${invoice.dueDate} - 用户: ${invoice.userId}`);
      });
      ...
      memoryDb.invoices.forEach(invoice => {
        if (invoice.status === 'sent' && invoice.dueDate) {
          ...
          if (now > dueDate) {
            console.log(`更新发票 ${invoice.invoiceNumber} 状态从 sent 到 overdue`);
      ...
      if (!type || type === 'overdue') {
        ...
        const overdueInvoices = memoryDb.invoices.filter(invoice => {
          console.log(`检查发票 ${invoice.invoiceNumber}:`);
          console.log(`  用户匹配: ${invoice.userId} === ${userId} ? ${invoice.userId === userId}`);
          console.log(`  状态匹配: ${invoice.status} in ['sent', 'overdue'] ? ${['sent', 'overdue'].includes(invoice.status)}`);
          console.log(`  有到期日期: ${!!invoice.dueDate}`);
          ...
          console.log(`  是否逾期: ${now} > ${dueDate} ? ${isOverdue}`);
          return isOverdue;
        }).slice(0, 5);

        console.log('找到逾期发票数量:', overdueInvoices.length);
        ...
        overdueInvoices.forEach(invoice => {
          ...
          if (daysOverdue > 0) {
            ...
            notifications.push({
              ...
              message: `Invoice ${invoice.invoiceNumber} is overdue by ${daysOverdue} day${daysOverdue > 1 ? 's' : ''}`,
              priority: 'high',
              data: {
      ...
      if (!type || type === 'due_soon') {
        ...
        dueSoonInvoices.forEach(invoice => {
          ...
          notifications.push({
            ...
            message: `Invoice ${invoice.invoiceNumber} is due in ${daysUntilDue} day${daysUntilDue > 1 ? 's' : ''}`,
            priority: 'medium',
            data: {
              invoiceId: invoice.id,
              invoiceNumber: invoice.invoiceNumber,
      ...
      if (!type || type === 'overdue') {
        ...
        overdueInvoices.forEach(invoice => {
          const daysOverdue = Math.floor((now - new Date(invoice.dueDate)) / (1000 * 60 * 60 * 24));
          // Only add if actually overdue (positive days)
          if (daysOverdue > 0) {
            notifications.push({
              id: `overdue-${invoice.id}`,
              type: 'overdue',
      ...
      // Due soon invoice notifications (within 7 days) - exclude overdue invoices
      if (!type || type === 'due_soon') {
        ...
        dueSoonInvoices.forEach(invoice => {
          const daysUntilDue = Math.ceil((new Date(invoice.dueDate) - now) / (1000 * 60 * 60 * 24));
          // Only show if not overdue (daysUntilDue should be positive)
          if (daysUntilDue > 0) {
            notifications.push({
              id: `due-soon-${invoice.id}`,
              type: 'due_soon',
    ...
    const filteredNotifications = notifications.filter(notification => {
      return !memoryDb.deletedNotifications.some(deleted => 
        deleted.userId === userId && deleted.notificationId === notification.id
      );
    });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // 检查是否已经删除过
    const alreadyDeleted = memoryDb.deletedNotifications.some(deleted => 
      deleted.userId === userId && deleted.notificationId === notificationId
    );
    
    if (!alreadyDeleted) {
      memoryDb.deletedNotifications.push({
        userId,
        notificationId,
        deletedAt: new Date()
      });
    ...
    console.log(`用户 ${userId} 删除通知: ${notificationId}`);
    console.log('已删除通知列表:', memoryDb.deletedNotifications);
