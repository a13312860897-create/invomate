...
import { FiEdit, FiDownload, FiTrash2, FiCheck, FiX, FiClock } from 'react-icons/fi';
import { useTranslation } from 'react-i18next';

const MobileInvoiceList = ({ invoices, onDelete, onExport, formatters }) => {
  const { t } = useTranslation(['common', 'invoices', 'routes']);

  const getStatusBadge = (status) => {
    const statusConfig = {
      draft: { color: 'bg-gray-100 text-gray-800 dark:bg-blue-900 dark:text-blue-200', icon: FiEdit, text: t('invoices:status.draft') },
      sent: { color: 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-200', icon: FiClock, text: t('invoices:status.sent') },
      paid: { color: 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-200', icon: FiCheck, text: t('invoices:status.paid') },
      overdue: { color: 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-200', icon: FiX, text: t('invoices:status.overdue') },
    };
  ...
  <div className="mobile-card-actions">
    <button
      onClick={() => onExport && onExport(invoice.id, invoice.invoiceNumber)}
