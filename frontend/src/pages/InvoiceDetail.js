...
import { FiArrowLeft, FiEdit, FiPrinter, FiCheck, FiX, FiMail, FiDollarSign, FiCalendar, FiCreditCard, FiSend } from 'react-icons/fi';
import { useAuth } from '../context/AuthContext';
import { useTranslation } from 'react-i18next';
import { toast } from 'react-hot-toast';
import InvoiceHeader from '../components/InvoiceHeader';
import PDFEmailSender from '../components/PDFEmailSender';
...
const InvoiceDetail = () => {
  ...
  const [showPDFEmailSender, setShowPDFEmailSender] = useState(false);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `invoice-${invoice.invoiceNumber || invoice.number || id}.pdf`);
      ...
      if (err.response) {
        if (err.response.status === 404) {
          toast.error('Invoice not found or PDF generation failed');
        } else if (err.response.status === 500) {
          toast.error('Server error, please contact administrator');
        } else {
          toast.error('PDF download failed, please try again later');
        }
      } else {
  ...
  const handleSendReminder = async () => {
    try {
      // 使用新的PDF邮件发送功能
      await api.post('/pdf-email/send/' + id, {
        recipientEmail: 'a13312860897@163.com', // 测试邮箱
        subject: `发票提醒 ${invoice?.invoiceNumber || id}`,
        message: '这是您的发票提醒。'
      });
  ...
  const handleSendEmail = async (method = emailMethod) => {
    ...
    try {
      ...
      const response = await api.post('/ai/send-invoice-email', { 
        invoiceId: id,
        recipientEmail: invoice?.client?.email || 'a13312860897@163.com',
        type: 'invoice',
        attachPDF: true,
        useUserConfig: true
      });
      ...
      // 显示成功消息
      toast.dismiss(loadingToast);
      if (response.data.data?.usedCustomConfig) {
        toast.success('Email sent successfully (using your email configuration)');
      } else {
        toast.success('Email sent successfully (using system default configuration)');
      }
      ...
      // 如果是邮件配置相关错误，提示用户配置邮件
      if (errorMessage.includes('邮件配置') || errorMessage.includes('SMTP') || errorMessage.includes('传输器') ||
          errorMessage.includes('email configuration') || errorMessage.includes('SMTP') || errorMessage.includes('transporter')) {
        errorMessage += '. Please go to the Email Settings page to configure your mailbox.';
      }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'draft':
        return <span className="status-badge status-draft">{t('draft')}</span>;
      case 'sent':
            ...
            <div>
              ...
              <div className="text-sm text-gray-700">
                {user?.logo && (
                  <div className="mb-3">
                    <img 
src={`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/${user.logo}`}
                      alt="Company Logo" 
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {/* 优先显示InvoiceItems，如果为空则尝试解析items JSON字段 */}
                  {(() => {
                    let itemsToDisplay = [];
                    
                    // 首先尝试使用InvoiceItems关联数据
                    if (invoice.InvoiceItems && Array.isArray(invoice.InvoiceItems) && invoice.InvoiceItems.length > 0) {
                      itemsToDisplay = invoice.InvoiceItems;
                    } 
                    // 如果InvoiceItems为空，尝试解析items JSON字段
                    else if (invoice.items) {
                      try {
                        const parsedItems = typeof invoice.items === 'string' ? JSON.parse(invoice.items) : invoice.items;
                        if (Array.isArray(parsedItems) && parsedItems.length > 0) {
                          itemsToDisplay = parsedItems.map((item, index) => ({
                            id: `json-${index}`,
...
{t('common:edit')}
            ...
            >
              <FiDollarSign className="-ml-1 mr-2 h-5 w-5" />
              Download PDF
            </button>
            <button
              type="button"
            ...
            <button
              type="button"
              className="inline-flex items-center px-4 py-2 border border-blue-300 shadow-sm text-sm font-medium rounded-md text-blue-700 bg-blue-50 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              onClick={() => setShowPDFEmailSender(true)}
            >
