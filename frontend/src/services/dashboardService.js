...
class DashboardService {
  ...
  async getInvoiceStatusDistribution() {
    try {
      const currentMonth = getCurrentDisplayMonth();
      console.log('获取发票状态分布，月份:', currentMonth);
      const response = await api.get(`/dashboard/unified-chart-data?month=${currentMonth}`);
      console.log('发票状态分布API响应:', response.data);
      
      // 修正数据访问路径
      const statusDistribution = response.data?.data?.statusDistribution || {};
      console.log('提取的状态分布数据:', statusDistribution);
  ...
  async getMonthlyRevenueTrend() {
    try {
      const currentMonth = getCurrentDisplayMonth();
      console.log('获取月度收入趋势，月份:', currentMonth);
      const response = await api.get(`/dashboard/unified-chart-data?month=${currentMonth}`);
      console.log('收入趋势API响应:', response.data);
      
      // 修正数据访问路径
      const revenueTrend = response.data?.data?.revenueTrend || {};
      console.log('提取的收入趋势数据:', revenueTrend);
      return revenueTrend;
    } catch (error) {
  ...
  async getNotifications(limit = 10) {
    try {
      ...
      console.log('请求URL:', '/notifications');
